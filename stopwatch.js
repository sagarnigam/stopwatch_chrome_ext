...
import { saveStopwatchState, removeStopwatchFromStorage, updateStopwatchState, saveStopwatchCount, getStopwatchCount } from './storage.js';

let stopwatchCount = getStopwatchCount();
...
export function createStopwatch() {
  ...
  saveStopwatchCount(stopwatchCount);
  ...
  saveStopwatchState(stopwatchData);
    ...
    let countdownInterval = setInterval(() => {
      ...
      if (countdown > 0) {
        ...
        stopwatchInterval = setInterval(function () {
          ...
          updateStopwatchState(stopwatchId, {
  ...
  function stopStopwatch() {
    ...
    stopBtn.disabled = true;
    // Update storage with stopped state
    updateStopwatchState(stopwatchId, {
      elapsedTime: elapsedTime,
      isRunning: false,
      startTime: 0
  ...
  function removeStopwatch() {
    clearInterval(stopwatchInterval);
    removeStopwatchFromStorage(stopwatchId);
...
/**
 * Restore stopwatch from saved state
 * @param {Object} stopwatchData - The saved stopwatch data
 */
export function restoreStopwatch(stopwatchData) {
  stopwatchCount++;
  ...
  const stopwatchName = stopwatchData.name || '';
  let nameDiv = null;
  if (stopwatchName) {
    nameDiv = createElement('div', { className: 'stopwatch-name', textContent: stopwatchName });
    stopwatchDiv.appendChild(nameDiv);
  ...
  // Stopwatch logic
  let startTime = stopwatchData.startTime || 0;
  let elapsedTime = stopwatchData.elapsedTime || 0;
  let stopwatchInterval;
  let isRunning = stopwatchData.isRunning || false;
  ...
  if (isRunning && startTime > 0) {
    // If it was running, calculate the new elapsed time based on when it was started
    const now = Date.now();
    ...
    // Enable reset button if there's elapsed time
    if (elapsedTime > 0) {
      resetBtn.disabled = false;
    }
  }
